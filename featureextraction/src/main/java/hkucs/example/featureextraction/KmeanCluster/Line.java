...
public class Line {
    ...
    public Line(double points[], Bitmap frame){
        this.points = points;
        theta = computeTheta();
        rtho = computeRtho(frame);
    }
    ...
    private double computeRtho(Bitmap frame){
        ...
        int mapHeight = frame.getHeight();
        int mapWidth = frame.getWidth();
        double d = Math.sqrt(Math.pow(mapHeight, 2) + Math.pow(mapWidth, 2));
